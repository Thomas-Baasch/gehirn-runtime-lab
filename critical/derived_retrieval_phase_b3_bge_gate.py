...
from governance.canon_router import EpistemicStatus, KnowledgeRecord, KnowledgeType, Sensitivity
...
CONTRACT_DRIVE_ID = "1oy8qVNyDbNJeLhkrrlexj50--1GSNJTChFWOypY94Ek"
CONTRACT_FILE_SHA256 = "441f05dcf1f85a23501e27e3644c72b2c814c8832558f0a35bb6b39638baa12a"
CONTRACT_SEMANTIC_SHA256 = "b5bfaad5e3d3ff19af4fc55b840f5a3ae150b9ee91848840f70d5438674e5333"
RERANKER = "BAAI/bge-reranker-v2-m3"
RERANKER_REVISION = "953dc6f6f85a1b2dbfca4c34a2796e7dde08d41e"
PURPOSE = "cross_project_memory"
TOP_N = 10
OUT = Path("reports/value/derived_retrieval_phase_b3_bge.json")
EXPECTED_DENSE = {
    "Recall@5": 1.0,
    "MRR@10": 0.538542,
    "nDCG@10": 0.654845,
...
def load_bge_reranker():
    ...
    return tokenizer, model, {
        ...
        "max_rss_mib_before_load": rss_before,
        "max_rss_mib_after_load": max_rss_mib(),
    }


def score_pairs(tokenizer, model, pairs: list[tuple[str, str]], batch_size: int = 8) -> tuple[list[float], dict]:
    import torch
    ...
    return scores, {
        ...
        "max_rss_mib_before_scoring": rss_before,
        "max_rss_mib_after_scoring": max_rss_mib(),
    }
...
def write_blocked(result: str, error: Exception, versions: dict, benchmark: dict) -> int:
    report = {
        "schema": "externes-gehirn.derived-retrieval-phase-b3-bge-evidence",
        "version": "0.1.0",
        "result": result,
        "error": f"{type(error).__name__}:{error}",
        "contract": {
            "drive_id": CONTRACT_DRIVE_ID,
            "file_sha256": CONTRACT_FILE_SHA256,
            "semantic_sha256": CONTRACT_SEMANTIC_SHA256,
        },
        "benchmark_sha256": BENCHMARK_SHA256,
        "benchmark_counts": {"records": benchmark.get("corpus_count"), "queries": benchmark.get("query_count")},
        "runtime_versions": versions,
        "max_rss_mib_at_failure": max_rss_mib(),
...
def main() -> int:
    ...
    try:
        ...
        tokenizer, reranker, reranker_evidence = load_bge_reranker()
    except MemoryError as exc:
        return write_blocked("BLOCKED_RESOURCE_OPERABILITY_EVIDENCE", exc, versions, benchmark)
    except OSError as exc:
        text = str(exc).lower()
        result = "BLOCKED_RESOURCE_OPERABILITY_EVIDENCE" if any(token in text for token in ("memory", "space", "disk", "resource")) else "BLOCKED_EXACT_MODEL_REVISION_UNAVAILABLE"
        return write_blocked(result, exc, versions, benchmark)
    except Exception as exc:
        return write_blocked("BLOCKED_EXACT_MODEL_REVISION_UNAVAILABLE", exc, versions, benchmark)

    embedder = FrozenEmbeddingProvider(vectors)
    with tempfile.TemporaryDirectory(prefix="eg-phase-b3-") as td:
        root = Path(td)
        ...
        try:
            scores, scoring_evidence = score_pairs(tokenizer, reranker, pairs, batch_size=8)
        except MemoryError as exc:
            index.close()
            return write_blocked("BLOCKED_RESOURCE_OPERABILITY_EVIDENCE", exc, versions, benchmark)
        ...
        # Safety 1: unauthorized project blocked before index/reranker.
        ...
        project_denied_before_dense_or_reranker = p01_status == "BLOCKED" and p01_hits == [] and len(index.query_log) == query_log_before

        # Safety 2: low clearance never sees confidential candidate.
        query_log_before = len(index.query_log)
        ...
        # Safety 3: derived-only rogue cannot be hydrated and therefore cannot reach reranker.
        rogue = KnowledgeRecord(
            record_id="11111111-2222-5333-8444-555555555555",
            source_ref="synthetic:phase-b3:rogue-index-only",
        ...
        index_only_never_reaches_reranker = rogue_status == "ALLOWED" and canonical.get(rogue.record_id) is None and all(record.record_id != rogue.record_id for record in rogue_hits)

        # Safety 4: reranking does not mutate Canon/history.
        ...
        for _ in range(3):
            score_pairs(tokenizer, reranker, probe_pairs, batch_size=8)
        rerank_side_effect_free = canonical.count() == canonical_count_before and canonical.events() == events_before and canonical.get(anchor_id) == anchor_before
        index.close()
    ...
    report = {
        ...
        "architecture": "policy -> DirectQdrant dense top10 -> BGE v2-m3 rerank -> Canon records",
        ...
        "mmarco_b2_baseline": MMARCO_BASELINE,
        "reranked": {
        ...
        "improvement_bge_minus_dense": {name: round(reranked_aggregate[name] - dense_aggregate[name], 6) for name in ("Recall@5", "MRR@10", "nDCG@10", "Top1", "Recall@10")},
        "improvement_bge_minus_mmarco": {name: round(reranked_aggregate[name] - MMARCO_BASELINE[name], 6) for name in ("Recall@5", "MRR@10", "nDCG@10", "Top1", "Recall@10")},
        "safety": safety,
        "scoring": scoring_evidence,
        "max_rss_mib_end": max_rss_mib(),
        "eligible_for_derived_role": eligible,
        "role_decision": "QUALIFIED_DIRECT_QDRANT_PLUS_BGE_RERANKER_DERIVED_ROLE" if eligible else "NO_SELECTION_BGE_MISSED_FROZEN_GATE",
        "scope_limit": "Derived retrieval role only; no final External Brain stack decision. GitHub-runner resource/latency evidence is not a production SLA.",
        "stack_decision": "NOT_MADE",
    }
    OUT.write_text(json.dumps(report, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    print(json.dumps({
        ...
        "mmarco": MMARCO_BASELINE,
        "bge": reranked_aggregate,
        "safety": safety,
        "reranker": reranker_evidence,
        "scoring": scoring_evidence,
