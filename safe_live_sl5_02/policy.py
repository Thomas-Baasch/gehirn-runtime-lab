...
ALLOWED_TARGETS = (PETER_REGISTRY, MENTORENRAT_REGISTRY, UNROUTED_INBOX)
ALLOWED_DELTA_TYPES = (
    "STATE_DELTA",
    "DECISION_DELTA",
    "RULE_DELTA",
    "OPEN_LOOP_DELTA",
    "EVIDENCE_LOCATOR_DELTA",
    "SUPERSESSION_CONFLICT_POINTER",
)
...
class Policy:
    ...
    allowed_targets: tuple[str, ...] = ALLOWED_TARGETS
    allowed_delta_types: tuple[str, ...] = ALLOWED_DELTA_TYPES
...
def evaluate(policy: Policy, target: TargetSnapshot, delta: DeltaCandidate) -> Decision:
    ...
    if len(delta.statement.strip()) == 0:
    ...
    if delta.delta_key in target.existing_delta_keys:
        return Decision(False, False, "NOOP", "DUPLICATE_DELTA_KEY", None)
    ...
    return Decision(
        ...
        action="PREPARE_APPEND_DRY_RUN_ONLY",
